// <auto-generated>
// 由 tools 脚本从 packages/quizsync_core/lib/ai/prompt.dart 生成（见 windows/README.md）。
// **不要手改**：prompt 是逐字契约，改了必须同时改 Dart 与 ai-contract.md，
// 并由 PromptVersionTests 的哈希比对兜住。
// 生成时的哈希：v1-c63f8999
// </auto-generated>
...
{
    /// <summary>`ai-contract.md` 第 2 节的 prompt 全文（与 Dart 侧逐字一致）。</summary>
...
"""";
    ...
    /// <summary>JSON 解析失败后附加的严格提醒（容错第 4 步）。</summary>
    public const string StrictJsonReminder = "上一次的返回不是合法 JSON。请只输出 JSON 对象本身，不要任何其他文字或代码围栏。";

    /// <summary>`v1-&lt;sha256 前 8 位&gt;`：prompt 变了版本号就变（与 Dart 侧同算法）。</summary>
    public static string ComputePromptVersion()
    {
        var digest = Convert.ToHexStringLower(SHA256.HashData(Encoding.UTF8.GetBytes(AiPrompt)));
        return $"v1-{digest[..8]}";
    }
}
